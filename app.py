...
def clip(orig):
    ...
    except Exception as e: return jsonify({"status": "fail", "error": f"Bad JSON – {e}"}), 400
    if start >= end: return jsonify({"status": "fail", "error": "start >= end"}), 400
    in_path = os.path.join(RECDIR, orig)
    if not os.path.exists(in_path): return jsonify({"status": "fail", "error": "Original not found"}), 404
    out_name = datetime.datetime.now().strftime("clip_%Y%m%d_%H%M%S.webm")
...
def link_public(fname):
    ...
    if request.method == "GET":
        for t, f in public_links.items():
            if f == fname: return jsonify({"status": "ok", "url": urljoin(request.url_root, f"public/{t}")})
...
def delete_file(fname):
    ...
    if not os.path.exists(fp): return jsonify({"status": "fail", "error": "not found"}), 404
    os.remove(fp)
    for token, flist in user_sessions.items():
        if fname in flist: flist.remove(fname)
...
def sitemap():
    ...
    def url(loc, prio="0.8"): return f"  <url>\n    <loc>{loc}</loc>\n    <lastmod>{today}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>{prio}</priority>\n  </url>"
    urls = [url(f"{base}/", "1.0")]
    global public_links
    public_links = _load_json(LINKS_FILE)
